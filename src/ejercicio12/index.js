...
myArray.forEach(function(item){
    ...
    // If the difference (module) between "item" and "14" is 0 then execute
    if(item % 14 === 0) {
        console.log(`The number ${item} is divisible by 14`);
    } 
});


// This function has an array and a divider as parameters.
const isDivisible = (array, divider) => {
    array.forEach(function(item){
        // If the difference (module) between "item" and "divider" is 0 then execute
        if(item % divider === 0) {
            console.log(`The number ${item} is divisible by ${divider}`);
        }  
    });
}

// Example
isDivisible(myArray, 23);